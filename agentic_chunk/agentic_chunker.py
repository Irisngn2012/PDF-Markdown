...
class AgenticChunker:
    ...
    def _find_relevant_chunk(self, proposition):
        ...
        print("DEBUG: extraction_found =>", extraction_found)  # Debug info

        # If the chain returns a dictionary with 'text' key
        if isinstance(extraction_found, dict):
            # 'text' might be a list of pydantic objects
            text_list = extraction_found.get("text", [])
            if text_list:
                # The first item might be a `ChunkID` object with an attribute chunk_id
                first_item = text_list[0]
                if hasattr(first_item, "chunk_id"):
                    potential_id = first_item.chunk_id
                    # If we got a valid chunk_id, check its length
                    if potential_id and len(potential_id) == self.id_truncate_limit:
                        return potential_id
            # If we reach here, no valid chunk id found
            return None

        # Alternatively, if the chain just returns a dict like {"chunk_id": "abc12"}:
        if isinstance(extraction_found, dict) and "chunk_id" in extraction_found:
            potential_id = extraction_found["chunk_id"]
            if potential_id and len(potential_id) == self.id_truncate_limit:
                return potential_id
            return None

        # Otherwise, it might be None or an unexpected format
        return None
